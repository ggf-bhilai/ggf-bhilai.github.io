document.addEventListener("DOMContentLoaded", function () {
  ...
  images.forEach(function (file) {
    ...
    var categories = parts.slice(1).map(function(cat) {
      return cat.trim();
    }).filter(function(cat) {
      return cat.length > 0;
    });
...
var seoByCategory = {
  ...
  function render() {
    ...
    products.forEach(function (p) {
      ...
      if (renderedFiles.indexOf(p.file) !== -1) {
        return;
      }
      renderedFiles.push(p.file);
...
card.setAttribute("data-file", p.file);
      ...
      var categoryBadges = p.allCategories.map(function(cat) {
      ...
      card.innerHTML =
        ...
        '<div class="categories">' + categoryBadges + '</div>' +
  ...
  uniqueProducts.forEach(function (p, index) {
